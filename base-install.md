...
```bash
pacstrap /mnt $kernel_pkg linux-firmware util-linux ufw tlp $cpu_microcode_pkg base base-devel btrfs-progs polkit go wget curl git openssh man-db sudo $editor_pkg $network_pkg $bootloader_pkg
```
